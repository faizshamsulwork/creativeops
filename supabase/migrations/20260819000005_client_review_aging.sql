-- ============================================================================
-- Copied verbatim from repo root: supabase-client-review-aging.sql
-- Client Review aging automation: workspace settings table, more creative_requests columns, working-day helper functions, move_client_review_to_awaiting() and run_client_review_aging_check() RPCs.
-- ============================================================================
...
-- Optional Supabase Cron setup — INTENTIONALLY OMITTED from this local migration.
-- The original block (repo root supabase-client-review-aging.sql, same section) nests a `$$...$$`
-- dollar-quoted string inside an `execute $cron$...$cron$` inside an outer `do $$...$$` block. Real
-- Postgres parses that fine, but the Supabase CLI's local migration runner mis-splits it at the
-- inner `$$`, aborting the whole migration with a syntax error. It's also explicitly optional
-- (pg_cron for an hourly automated aging sweep — the app's own "Run Aging Check" admin action calls
-- run_client_review_aging_check() directly and works without it). Production's copy of this file is
-- unaffected; only this local migration excludes the cron scheduling.
